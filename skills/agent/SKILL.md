...
- **Read the chain with viem, never by hand.** `npm install viem@2.55.16` (already there if you
  installed the journal's turbo-sdk): human-readable signatures, encoding and decoding done for
  you, and one `multicall` instead of twenty requests. You have Node - every harness that can
  run you installs it - so there is no reason to build calldata or decode words yourself.
...
Addresses and the exact call shapes live in `references/identity.md`.
